          ...
          return (
            <Link href={`/products/${product.id}`} key={product.id} style={{ 
              ...
              <div style={{ width: '100%', height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                <img src={imgUrl.startsWith('/') ? `${process.env.NEXT_PUBLIC_API_URL.replace('/api', '')}${imgUrl}` : imgUrl} alt={product.title} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} onError={(e) => { e.target.onerror = null; e.target.src = FALLBACK_IMAGE; }} />
              </div>
